...
if command -v /usr/local/bin/speedtest-go >/dev/null 2>&1; then
  ...
  PKT_LOSS=$(echo "$RESULT" | awk '/Packet Loss:/ {print $3}')
elif command -v /usr/local/bin/speedtest-cli >/dev/null 2>&1; then
  RESULT=$(/usr/local/bin/speedtest-cli --secure --simple 2>/dev/null)
  DOWNLOAD=$(echo "$RESULT" | awk '/Download:/ {print $2 " " $3}')
  UPLOAD=$(echo "$RESULT" | awk '/Upload:/ {print $2 " " $3}')
  PKT_LOSS="N/A"
else
